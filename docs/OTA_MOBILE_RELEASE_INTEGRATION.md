...
> - **Mobile product split (2026-07-31, supersedes the previous bullet's slugs/spellings — see `docs/superpowers/plans/2026-07-31-mobile-product-split.md`):** `autopilot` is now BACKEND-ONLY; the new **`mobile`** product owns builds + the whole OTA family (`MB_*` wire names, constructor == wire, e.g. `MB_MOBILE_DISPATCH`; `OTA_*` unchanged). Grants live under `product_slug='mobile'` (product-level = fleet-wide — the `mobile/*` wildcard is retired; per-app keys unchanged). `aliasGrantSlugs "airborne-ota" = ["mobile"]`, and the route gate also honors PRODUCT-level alias grants (a fleet-wide mobile Admin opens `/airborne/*` with zero scoped rows). Migration `0049` seeds mobile roles, mirrors product-level autopilot grants, collapses wildcards (greatest role wins), re-slugs+re-points per-app rows, and moves unambiguous overrides with the `MB_` rename. New: `POST /mobile/releases/:id/abort` (`MB_RELEASE_ABORT`, MobileBuild-only) so scoped mobile grants can cancel their own builds without any autopilot access.
...
## 5.8 RBAC — the `mobile` product (revised 2026-07-31, product split)

- **Two products, disjoint universes.** `autopilot` is BACKEND-ONLY (BE release lifecycle, config, stagger, AI). The **`mobile`** product owns builds + OTA: `allPermissions Mobile` = the `MB_*` family (`MB_RELEASE_VIEW/CREATE/PROMOTE/ROLLOUT/REVERT/ABORT`, `MB_MOBILE_DISPATCH`, `MB_MOBILE_APP_MANAGE`, `MB_AI_SUMMARIZE`) + the whole `OTA_*` family. System **Admin/Manager/Viewer** tier both (Manager lacks `MB_MOBILE_APP_MANAGE`, `OTA_RELEASE_DISCARD`, `OTA_APP_MANAGE`; Viewer = `MB_RELEASE_VIEW` + `OTA_VIEW`). Wire names: `MB_*` == constructor (like `OTA_*`); a mobile grant structurally CANNOT confer BE access — BE verbs don't exist in its vocabulary and BE gates check a slug it has no rows for.
- **One grant covers both surfaces.** `sc_person_deployment_access (mobile, "<name>/<platform>")` for one app; a **product-level `mobile` grant** for the fleet (the `mobile/*` wildcard is retired — migration `0049` collapsed those rows, greatest role wins). Precedence: exact app key > product-level baseline (`deploymentRowFor`, exact-only).
- **Build verbs** enforced per app row in the handlers (`requireAppPerm[All]`); reads are visible-but-inert. `MB_MOBILE_APP_MANAGE`: catalog edit is per-app, catalog create and fleet enumeration are product-level-only (`requireProductPerm`). Cancel-a-build is `POST /mobile/releases/:id/abort` (`MB_RELEASE_ABORT`, MobileBuild rows only, per-app scoped) — the shared autopilot update route is no longer needed for mobile cancels.
- **OTA verbs** accept the legacy per-ref airborne-ota grant OR the mobile grant (`requireOtaPermission` / `requireOtaPerm`; `aliasGrantSlugs "airborne-ota" = ["mobile"]`). The route-level product gate honors PRODUCT-level alias access too — a fleet-wide mobile Admin opens `/airborne/*` with zero scoped rows. Legacy per-ref grants remain the escape hatch for refs with no catalog row; the admin UI offers `autopilot (backend)` and `mobile (builds · OTA)`, never airborne-ota.
- **Frontend gating**: `/airborne/access` (per-ref sets, mobile-grant-aware) for OTA panels; `GET /mobile/access` plus `hasPermission('mobile', 'MB_*', "<name>/<platform>")` (exact key → product baseline; alias for airborne-ota) for mobile verbs; OTA fetches use `retry:false` and degrade the card on 403.
