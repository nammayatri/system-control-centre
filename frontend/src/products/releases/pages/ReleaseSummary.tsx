...
const ReleaseSummary: React.FC = () => {
  ...
  // Mobile builds get the dedicated summary page (after all hooks — rules of hooks).
  if (release.tracker_type === 'MobileBuild') return <MobileReleaseSummary />;
  ...
  return (
    <div className="flex flex-col flex-1 w-full pb-12">
      <div className="flex items-center text-sm text-zinc-500 font-medium mb-3 sm:mb-4 flex-wrap gap-y-1">
        <Link to="/backend/releases" className="hover:text-zinc-700 transition-colors duration-150">Releases</Link>
      ...
      <div className="flex flex-col gap-3 mb-4 sm:mb-5">
        <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
          ...
          {(release.release_context?.revert === 1 || release.revertsReleaseId) && <Badge variant="purple" dot>REVERT</Badge>}
          {release.sync_enabled === 'true' && (() => {
        ...
        <div className="flex items-center gap-2 flex-wrap sm:justify-end">
          ...
          <PermissionGate product="autopilot" permission="RELEASE_DELETE" appGroup={release.appGroup}>
            <SimpleTooltip content="Delete"><Button size="icon" variant="ghost" className="text-red-500 hover:bg-red-50" loading={deleteMut.isPending} onClick={() => doAction('delete', async () => { await deleteMut.mutateAsync(id!); navigate('/backend/releases'); }, true, 'Delete this release tracker permanently. This removes the audit trail and cannot be undone.')}><Trash2 className="w-4 h-4" /></Button></SimpleTooltip>
          </PermissionGate>
          <SimpleTooltip content="Clone"><Button size="icon" variant="ghost" onClick={() => navigate(`/backend/releases/${id}/clone`)}><Copy className="w-4 h-4" /></Button></SimpleTooltip>
      ...
      <div className="flex justify-end pt-5">
        <Button variant="secondary" onClick={() => navigate('/backend/releases')}>Back to Releases</Button>
      </div>
