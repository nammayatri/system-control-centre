...
import { fetchUser, deactivateUser, addProductAccess, removePermissionOverride } from '../../services/admin';
...
const UserDetail: React.FC = () => {
  ...
  if (isLoading) return <div className="p-10 text-center text-zinc-400">Loading user...</div>;
  ...
  return (
    <div className="flex flex-col w-full max-w-4xl">
      <div className="flex items-center justify-between mb-5">
        <div>
          <h1 className="text-lg font-bold text-zinc-800">{user.name || 'User'}</h1>
      ...
      <div className="bg-white rounded-lg border border-border p-6 mb-5">
        <h2 className="text-sm font-bold text-zinc-700 uppercase tracking-wider mb-4">User Information</h2>
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div><span className="text-zinc-400 text-xs">Name</span><div className="text-zinc-800 font-medium mt-0.5">{user.name || '-'}</div></div>
          <div><span className="text-zinc-400 text-xs">Email</span><div className="text-zinc-800 font-mono text-xs mt-0.5">{user.email}</div></div>
          <div><span className="text-zinc-400 text-xs">Status</span><div className="mt-0.5">{user.status || 'active'}</div></div>
          <div><span className="text-zinc-400 text-xs">Created</span><div className="text-zinc-800 font-mono text-xs mt-0.5">{user.createdAt || '-'}</div></div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg border border-border p-6 mb-5">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-sm font-bold text-zinc-700 uppercase tracking-wider">Product Access</h2>
        </div>
      ...
      <div className="bg-white rounded-lg border border-border p-6 mb-5">
        <h2 className="text-sm font-bold text-zinc-700 uppercase tracking-wider mb-4">Permission Overrides</h2>
