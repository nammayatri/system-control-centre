...
import { fetchRole, updateRole } from '../../services/admin';
...
const RoleDetail: React.FC = () => {
  ...
  if (isLoading) return <div className="p-10 text-center text-zinc-400">Loading role...</div>;
  ...
  return (
    <div className="flex flex-col w-full max-w-3xl">
      <div className="flex items-center justify-between mb-5">
        <div>
          <h1 className="text-lg font-bold text-zinc-800">{role.name}</h1>
      ...
      <div className="bg-white rounded-lg border border-border p-6">
        <h2 className="text-sm font-bold text-zinc-700 uppercase tracking-wider mb-4">Permissions ({selectedPerms.length})</h2>
        {allPermissions.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            {allPermissions.map((perm: string) => (
              <label key={perm} className="flex items-center gap-2.5 px-3 py-2 rounded-lg hover:bg-zinc-50 cursor-pointer transition-colors">
