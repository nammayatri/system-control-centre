...
import Products.Types (ProductSlug (..), productSlugToText, allPermissionsText)
import Core.Auth.Types (PersonAuth (..), PersonProductPerms (..))
import Core.Auth.Queries (findPersonById, findProductAccessForPerson, findAllProductsForPerson, findTokenByValue, TokenRow (..))
