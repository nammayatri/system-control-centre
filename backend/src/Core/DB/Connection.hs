...
ensureSchema db = withConn db $ \conn -> do
    ...
    _ <- execute_ conn "DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'server_config_name_unique') THEN ALTER TABLE server_config ADD CONSTRAINT server_config_name_unique UNIQUE (name); END IF; END $$"
