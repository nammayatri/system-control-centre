...
upsertProduct ::
  ...
  Maybe Bool ->
  m ()
upsertProduct productName' cluster' namespace' vsName' productType' productAcronym' syncCluster' needInfraApproval slackChannel' repoName' aiChangelogEnabled' = withCloudDb $ \cloud db -> do
  withConn db $ \conn ->
