...
doCreate cfg tracker mts targetCluster = do
    let base = normaliseBase (syncClusterUrl cfg)
        ...
        -- 'Actions.Release.createReleaseHBodyAfterClaim'), but this keeps
    ...
    case eitherDecode b :: Either String Value of
        Right (Object obj) ->
            case KM.lookup (AK.fromText "id") obj of
                ...
                _ -> KM.lookup (AK.fromText "message") obj >>= extractFromMessage
        _ -> Nothing
  where
    extractFromMessage (String msg) =
        case T.strip <$> T.stripPrefix "Tracker created:" msg of
            Just t | not (T.null t) -> Just t
            _ -> Nothing
    extractFromMessage _ = Nothing
