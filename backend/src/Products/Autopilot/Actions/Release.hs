...
import Products.Autopilot.Sync (buildAuthHeaders, normaliseBase, triggerImmediateRevertSync)
...
-- | Cross-cloud wrapper: computes this cluster's own diff, then — if this
-- instance has an outbound sync cluster configured ('syncClusterConfigured')
-- and the release carries a 'globalId' — fetches the linked release's diff
-- from the secondary cluster over HTTP and merges the two. Absent secondary
-- data (no sync cluster configured, no globalId, or the fetch failed) simply
-- means 'ccdSecondary' is Nothing; the primary diff is unaffected either way.
releaseDiffH :: AuthedPerson -> Text -> Maybe Text -> Flow CrossCloudDiffResponse
releaseDiffH _ap rid mType = do
  cfg <- getConfig
  m <- findReleaseTracker rid
  case m of
    Nothing -> pure $ CrossCloudDiffResponse (DiffResponse "" "" "Release not found") Nothing Nothing
    Just (tracker, mTargetState) -> do
      primary <- releaseDiffCore cfg rid tracker mTargetState
      mSecondary <- fetchSecondaryDiff cfg tracker mType
      pure $ CrossCloudDiffResponse primary (fst <$> mSecondary) (snd <$> mSecondary)

-- | Service-to-service counterpart of 'releaseDiffH', keyed by 'globalId'
-- instead of the local release id — this is what a primary cluster calls
-- into on the secondary cluster (see 'fetchSecondaryDiff') to pull its half
-- of a cross-cloud diff. Same-cloud only: does not itself recurse into
-- fetching a THIRD cluster's diff.
releaseDiffByGlobalIdH :: AuthedPerson -> Text -> Maybe Text -> Flow DiffResponse
releaseDiffByGlobalIdH _ap gid _mType = do
  cfg <- getConfig
  m <- findReleaseTrackerByGlobalId gid
  case m of
    Nothing -> pure $ DiffResponse "" "" "Release not found"
    Just (tracker, mTargetState) -> releaseDiffCore cfg (NT.releaseId tracker) tracker mTargetState

-- | Same-cloud diff for a release/globalId reachable from THIS cluster's own
-- DB — the core logic reused by both the user-facing diff endpoint and the
-- service-to-service global-id diff endpoint a peer cluster calls into.
releaseDiffCore :: Config -> Text -> ReleaseTracker -> Maybe TargetState -> Flow DiffResponse
releaseDiffCore cfg rid tracker mTargetState = do
...
-- | Fetch the linked release's diff from the secondary cluster, keyed by
-- 'globalId'. Returns Nothing (never throws) when: this instance has no
-- outbound sync cluster configured, the release has no globalId, the
-- product has no sync cluster configured, or the HTTP call/decode fails —
-- any of those simply means there's nothing cross-cloud to show, not an
-- error worth surfacing on the primary diff.
fetchSecondaryDiff :: Config -> ReleaseTracker -> Maybe Text -> Flow (Maybe (DiffResponse, Text))
fetchSecondaryDiff cfg tracker mType = do
  let syncConfigured = syncClusterEnabled cfg && not (null (syncClusterUrl cfg))
      mGid = NT.globalId tracker
  case (syncConfigured, mGid) of
    (True, Just gid) | not (T.null gid) -> do
      mProduct <- findProductByName (NT.appGroup tracker)
      case mProduct >>= getProductSyncCluster of
        Just cluster | not (T.null cluster) -> do
          let base = normaliseBase (syncClusterUrl cfg)
              url = base <> "release/diff/global/" <> gid <> maybe "" ("?type=" <>) mType
              ...
              req =
                (defaultReq url)
                  { reqMethod = GET,
                    ...
                    reqRetries = 1,
                    reqLogTag = "secondary-diff"
                  }
          result <- liftIO $ httpRaw req
          case result of
            Right HttpResponse {respStatus = s, respBody = b}
              | s < 400 ->
                  pure $ case A.eitherDecode b :: Either String DiffResponse of
                    Right dr -> Just (dr, cluster)
                    Left _ -> Nothing
            _ -> pure Nothing
        _ -> pure Nothing
    _ -> pure Nothing
