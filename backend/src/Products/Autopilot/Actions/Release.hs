        ...
        then handleConfigMapRevert tracker mTargetState rid
          ...
          if not oldDepExists && not (T.null ctxOldVersion) && ctxOldVersion /= "new" && ctxOldVersion /= "unknown"
            then pure $ APIResponse "ERROR" ("Old deployment not found in K8s: " <> oldDepName <> ". Cannot revert.")
              ...
              mSvcCfg <- findServiceByProductAndName (NT.appGroup tracker) (NT.service tracker)
              let decodedSteps raw = case decodeStrategyText raw of
                    Right steps | not (null steps) -> Just steps
                    _ -> Nothing
                  revertRolloutStrategy =
                    fromMaybe
                      (NT.rolloutStrategy tracker)
                      ( (mSvcCfg >>= S.dcRevertStrategy >>= decodedSteps)
                          <|> (mSvcCfg >>= S.dcRolloutStrategy >>= decodedSteps)
                      )
                  ...
                  revertedTracker =
                    (tracker :: ReleaseTracker)
                      { NT.releaseId = newRid,
                        ...
                        NT.endTime = Nothing,
                        NT.rolloutHistory = [],
                        NT.rolloutStrategy = revertRolloutStrategy,
