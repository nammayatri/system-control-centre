...
createConfigMapH ap body = do
  ...
  case extractCmFields body of
    ...
    Right (product', service', env', cluster', desc, changeLog', config', _releaseManager', priority', _scheduleTime', name') -> do
            ...
            object $
              concat
                [ maybe [] (\c -> ["config" .= c]) config',
                  ...
                  maybe [] (\sf -> ["secondary_file" .= sf]) secondaryFile'
                ]
          tracker =
            ReleaseTracker
              { releaseId = rid,
      ...
      void $ forkFlow $ void $ runConfigReview (apEmail ap) tracker False
  ...
  case m of
    ...
    Just (rt, mts) -> do
      ...
      let isRevert = case body of
            ...
            _ -> False
          isApprove = case body of
            Object obj -> isTruthy "is_approved" obj
            _ -> False
      if isRevert
        then handleConfigMapRevert rt mts cmId'
        else
          if isApprove && reviewBlocksApproval rt
            then
              pure $
                APIResponse
                  "ERROR"
                  "AI review flagged this config change as potentially breaking — acknowledge the warning in the AI Review tab before approving."
            else do
                  ...
                  case body of
                    Object obj -> do
                      case getStrM "status" obj of
                        ...
                        Just "COMPLETED" -> do
                          notifyConfigMapCompleted updated
                          cfg <- getConfig
                          when (NT.syncEnabled updated == Just "true" && not (null (syncClusterUrl cfg))) $
                            syncCompletedConfigMap cfg updated
                        Just "ABORTED" -> do
...
syncCompletedConfigMap :: Config -> ReleaseTracker -> Flow ()
syncCompletedConfigMap cfg rt = do
  let rawUrl = syncClusterUrl cfg
      normalised =
      ...
      meta = case NT.metadata rt of
        Just (Object o) -> o
        _ -> KM.empty
      fileVal = case KM.lookup (K.fromText "secondary_file") meta of
        Just (String sf) -> sf
        _ -> case KM.lookup (K.fromText "file") meta of
          Just (String f) -> f
          _ -> case KM.lookup (K.fromText "config") meta of
            Just (String c) -> c
            _ -> ""
      syncBody =
        object $
          [ "product" .= NT.appGroup rt,
            ...
            <> case KM.lookup (K.fromText "name") meta of
              Just (String n) -> ["name" .= n]
              _ -> []
    ...
    forkFlow $ do
      ...
      case syncResult of
        Right HttpResponse {respStatus = s, respBody = b} | s < 400 -> do
          ...
          insertReleaseEvent (NT.releaseId rt) "BUSINESS" "CONFIGMAP_SYNC_RESPONSE" (toJSON (T.pack (LBS.unpack b)))
        Right HttpResponse {respStatus = s, respBody = b} -> do
          logErrorG $ "[CONFIGMAP-SYNC] failed (HTTP " <> T.pack (show s) <> ")"
          insertReleaseEvent (NT.releaseId rt) "BUSINESS" "CONFIGMAP_SYNC_FAILED" (toJSON (T.pack (LBS.unpack b)))
        Left e -> do
          logErrorG $ "[CONFIGMAP-SYNC] exception: " <> T.pack (show e)
          insertReleaseEvent (NT.releaseId rt) "BUSINESS" "CONFIGMAP_SYNC_FAILED" (toJSON (T.pack (show e)))
