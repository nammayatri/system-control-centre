...
notifyComplete = do
  ...
  -- Sync to secondary cluster if isSync was set at creation time.
  when (syncEnabled rt == Just "true") $ do
    cfg <- getCfg
    when (not (null (syncClusterUrl cfg))) $
      lift $
        syncCompletedConfigMap cfg rt
