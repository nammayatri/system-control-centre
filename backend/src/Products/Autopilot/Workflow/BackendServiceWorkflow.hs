...
runAutoHealthGate :: Config -> K8sReleaseContext -> ReleaseTracker -> ReleaseStatus -> T.Text -> StateFlow Bool
runAutoHealthGate cfg ctx freshRT freshStatus stageLabel = do
  ...
  case promResult of
    PromAbort reason -> do
      ...
      currentRT' <- getRT
      currentTS' <- gets targetState
      casUpdateOrBail (stageLabel <> "/promAbort") currentRT' currentTS' freshStatus
  ...
  case unsnocList (rolloutHistory freshRT) of
    ...
    Just _ -> do
      ...
      casUpdateOrBail (stageLabel <> "/decisionHist") currentRTDR currentTSDR freshStatus
  ...
  when (combinedDecision /= Continue) $ do
    let reasonText = if null combinedReasons then Nothing else Just (T.intercalate "; " combinedReasons)
        -- Stage label folded into decisionType so the dedup key changes
        -- on every new stagger stage / the final gate, even when the
        -- decision/reason text repeats verbatim.
        stageDecisionType = "ROLLOUT_DECISION_" <> stageLabel
        notifyMsg =
          "⏸️ Decision: "
  ...
  case combinedDecision of
    ...
    Abort -> do
      ...
      currentRT' <- getRT
      currentTS' <- gets targetState
      casUpdateOrBail (stageLabel <> "/decisionAbort") currentRT' currentTS' freshStatus
...
rolloutLoop wfCfg cfg ctx currentIndex totalSteps stepStartTime iterCount loopStart = do
  ...
  case freshResult of
    ...
    Just (freshRT, freshMts) -> do
      ...
      touchReleaseHeartbeat (releaseId freshRT)
                    ...
                    AUTO -> do
                      ...
                      if fastForwarded
                        then do
                          ...
                          pure True
                        else runAutoHealthGate cfg ctx freshRT freshStatus ("STAGE_" <> T.pack (show currentIndex))
...
monitorHealth = do
  ...
  case waitResult of
    Left errMsg -> do
      ...
      insertReleaseEvent (releaseId rt) "BUSINESS" "MONITORING_PODS_NOT_READY" (toJSON errMsg)
      fetchAndLogPodLogs cfg ctx (releaseId rt)
  ...
  -- Full-traffic stability gate: reuses the SAME regular AB/HS decision run
  -- (already initiated once at release start and polled successfully
  -- through every prior stage) instead of the old post-monitoring
  -- mechanism, which initiated a brand-new "-post" run and polled it with
  -- zero grace period — the HS engine's own "run not indexed yet" 500 was
  -- then treated as a hard Abort by fail-closed, on every single release.
  ...
  when (postMonitoringEnabled && masterEnabled && perServiceEnabled) $ do
    ...
    when (cooloffMins > 0) $ do
      insertReleaseEvent
        (releaseId rt)
        "BUSINESS"
        "FULL_TRAFFIC_COOLOFF_STARTED"
        (object ["cooloffMinutes" .= cooloffMins])
      logInfoS $ "[WORKFLOW] Waiting " <> T.pack (show cooloffMins) <> "m cooloff at 100% before finalizing"
      threadDelay (Seconds (cooloffMins * 60))
    stabilizeAtFullTraffic cfg ctx wfCfg

  logInfoS "Health monitoring complete"

-- | After the last rollout step's cooloff at 100%, re-check the regular
-- AB/HS decision before letting the release finalize. Mirrors the mid-rollout
-- AUTO gate via 'runAutoHealthGate' — an Abort there already transitions
-- status to ABORTING and throws, unwinding through here so the runner's
-- abort machinery reverts traffic. Max 30 polls, same bail-out cap as the
-- rollout loop.
stabilizeAtFullTraffic :: Config -> K8sReleaseContext -> WorkflowConfig -> StateFlow ()
stabilizeAtFullTraffic cfg ctx wfCfg = go (0 :: Int)
  where
    go iteration = do
      rt <- getRT
      if iteration > 30
        then
          insertReleaseEvent
            (releaseId rt)
            "DECISION_ENGINE"
            "FULL_TRAFFIC_CHECK_TIMEOUT"
            (object ["iterations" .= iteration])
        else do
          shouldFinish <- runAutoHealthGate cfg ctx rt (status rt) "FULL_TRAFFIC"
          if shouldFinish
            then logInfoS "[WORKFLOW] Full-traffic stability check: CONTINUE"
            else do
              threadDelay (Seconds (wcCollectMetricsDelay wfCfg))
              go (iteration + 1)
    ...
    Right (K8sResult podName)
      | T.null (T.strip podName) ->
          insertReleaseEvent
            ...
            "BUSINESS"
            "POD_LOGS_FAILED"
            (object ["error" .= ("No pods found for deployment " <> deploymentName ctx)])
      ...
      case logsResult of
        ...
        Right (K8sResult logs) ->
          insertReleaseEvent
            ...
            "BUSINESS"
            "POD_LOGS"
            (object ["pod" .= pn, "deployment" .= deploymentName ctx, "logs" .= logs])
