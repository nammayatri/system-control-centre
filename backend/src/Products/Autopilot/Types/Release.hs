...
validateStatusTransition from to = to `elem` allowed from
  where
    ...
    allowed GCLT_ABORTED = [DISCARDED]
    allowed COMPLETED = []
    allowed DISCARDED = []
...
data ReleaseTracker = ReleaseTracker
  { releaseId :: Text,
    ...
    -- | Post-release AB outcome (UNASSIGNED | VERIFIED | MISSED_ABORT | FALSE_ABORT | TRUE_ABORT | INVALID).
    abValidationStatus :: Maybe Text,
    -- | Full AB validation object: {status, is_approved, rca_description, history:[...]}.
    abValidation :: Maybe Value
  }
  deriving (Eq, Show, Generic)
