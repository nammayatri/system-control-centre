...
-- | Clone deployment. @initialReplicas@ is baked into .spec.replicas directly
-- in the patch (rather than inherited from the source deployment and fixed up
-- by a follow-up scale call) so the new deployment is born at the right size
-- -- normally the first rollout stage's own podCount.
buildCloneDeploymentCommand :: Config -> K8sReleaseContext -> Int -> String
buildCloneDeploymentCommand cfg ctx initialReplicas =
    let sourceDep = T.unpack (serviceName ctx) <> "-" <> T.unpack (oldVersion ctx)
        targetDep = T.unpack (deploymentName ctx)
        explicitDockerImage = maybe "" T.unpack (dockerImage ctx)
        patchFilter = ".metadata.name = $targetDep | .metadata.labels.version = $newTag | .spec.replicas = $replicas | .spec.selector.matchLabels.version = $newTag | .spec.template.metadata.labels.version = $newTag | (.spec.template.spec.containers[0].image) |= (if ($dockerImage != \"\") then (if (($dockerImage | test(\"/\")) or ($dockerImage | test(\":\"))) then $dockerImage elif test(\":\") then sub(\":[^:]+$\"; \":\" + $dockerImage) elif test(\"-\" + $oldTag + \"$\") then sub(\"-\" + $oldTag + \"$\"; \"-\" + $dockerImage) elif test(\"-\") then sub(\"-(?<last>[^-:]+)$\"; \"-\" + $dockerImage) else . end) elif test(\"-\" + $oldTag + \"$\") then sub(\"-\" + $oldTag + \"$\"; \"-\" + $newTag) elif test(\":\" + $oldTag + \"$\") then sub(\":\" + $oldTag + \"$\"; \":\" + $newTag) elif test(\":\") then sub(\":[^:]+$\"; \":\" + $newTag) elif test(\"-\") then sub(\"-(?<last>[^-:]+)$\"; \"-\" + $newTag) else . end) | del(.metadata.uid,.metadata.resourceVersion,.metadata.generation,.metadata.creationTimestamp,.metadata.managedFields,.metadata.annotations.\"deployment.kubernetes.io/revision\",.status)"
     in unwords [kubectlBin cfg, "-n", shellQuote (namespace ctx), "get deployment", shellQuote (T.pack sourceDep), "-o json | jq", "--arg targetDep", shellQuote (T.pack targetDep), "--arg container", shellQuote (containerName ctx), "--arg newTag", shellQuote (newVersion ctx), "--arg oldTag", shellQuote (oldVersion ctx), "--arg dockerImage", shellQuote (T.pack explicitDockerImage), "--argjson replicas", shellQuote (T.pack (show (max 1 initialReplicas))), "'" <> patchFilter <> "'", "|", kubectlBin cfg, "-n", shellQuote (namespace ctx), "apply -f -"]

-- | Clone deployment, replacing containers[0].env with the supplied envs. See
-- 'buildCloneDeploymentCommand' for why @initialReplicas@ is baked into the patch.
buildCloneDeploymentWithEnvsCommand :: Config -> K8sReleaseContext -> Text -> Int -> String
buildCloneDeploymentWithEnvsCommand cfg ctx envsJson initialReplicas =
    let sourceDep = T.unpack (serviceName ctx) <> "-" <> T.unpack (oldVersion ctx)
        targetDep = T.unpack (deploymentName ctx)
        explicitDockerImage = maybe "" T.unpack (dockerImage ctx)
        patchFilter = ".metadata.name = $targetDep | .metadata.labels.version = $newTag | .spec.replicas = $replicas | .spec.selector.matchLabels.version = $newTag | .spec.template.metadata.labels.version = $newTag | (.spec.template.spec.containers[0].image) |= (if ($dockerImage != \"\") then (if (($dockerImage | test(\"/\")) or ($dockerImage | test(\":\"))) then $dockerImage elif test(\":\") then sub(\":[^:]+$\"; \":\" + $dockerImage) elif test(\"-\" + $oldTag + \"$\") then sub(\"-\" + $oldTag + \"$\"; \"-\" + $dockerImage) elif test(\"-\") then sub(\"-(?<last>[^-:]+)$\"; \"-\" + $dockerImage) else . end) elif test(\"-\" + $oldTag + \"$\") then sub(\"-\" + $oldTag + \"$\"; \"-\" + $newTag) elif test(\":\" + $oldTag + \"$\") then sub(\":\" + $oldTag + \"$\"; \":\" + $newTag) elif test(\":\") then sub(\":[^:]+$\"; \":\" + $newTag) elif test(\"-\") then sub(\"-(?<last>[^-:]+)$\"; \"-\" + $newTag) else . end) | (.spec.template.spec.containers[] | select(.name == $container) | .env) = ($envs | fromjson) | del(.metadata.uid,.metadata.resourceVersion,.metadata.generation,.metadata.creationTimestamp,.metadata.managedFields,.metadata.annotations.\"deployment.kubernetes.io/revision\",.status)"
     in unwords [kubectlBin cfg, "-n", shellQuote (namespace ctx), "get deployment", shellQuote (T.pack sourceDep), "-o json | jq", "--arg targetDep", shellQuote (T.pack targetDep), "--arg container", shellQuote (containerName ctx), "--arg newTag", shellQuote (newVersion ctx), "--arg oldTag", shellQuote (oldVersion ctx), "--arg dockerImage", shellQuote (T.pack explicitDockerImage), "--arg envs", shellQuote envsJson, "--argjson replicas", shellQuote (T.pack (show (max 1 initialReplicas))), "'" <> patchFilter <> "'", "|", kubectlBin cfg, "-n", shellQuote (namespace ctx), "apply -f -"]
...
buildPatchDeploymentEnvsCommand cfg ctx envsJson =
    -- jq parses envsJson into a real JSON array (not a string) before patching.
    let stripAndPatch =
            "($envs | fromjson) as $filtered | [{\"op\":\"replace\",\"path\":\"/spec/template/spec/containers/0/env\",\"value\":$filtered}]"
     in unwords
            [ kubectlBin cfg
            , "-n"
