...
data DecisionEngineHSStatus
  = Uninitiated | ConfigFound | Started | Running | Stopped | AbHsException
  deriving (Eq, Show, Read, Generic)
instance ToJSON DecisionEngineHSStatus
instance FromJSON DecisionEngineHSStatus

data PodsScaleDownStatus
  = ScaleDownScheduled | ScaleDownInProgress | ScaleDownAborted
  | ScaleDownCompleted | ScaleDownDiscarded
  deriving (Eq, Show, Read, Generic)
...
data BackendServiceWFStatus
  = BSInit                      -- ^ Initial validation
  | BSCreateDeployment          -- ^ Create K8s Deployment
  | BSUpdateService             -- ^ Update K8s Service
  | BSApplyConfigMap            -- ^ Apply ConfigMaps
  | BSApplyDestinationRule      -- ^ Apply Istio DestinationRule
  | BSFlipVirtualService        -- ^ Update Istio VirtualService for traffic
  | BSProgressiveRollout        -- ^ Progressive traffic shift (0->25->50->100)
  | BSMonitoring                -- ^ Monitor pod health and metrics
  | BSStabilize                 -- ^ Stabilization period
  | BSScaleDownOld              -- ^ Scale down old deployment
  | BSDone                      -- ^ Complete
  | BSRollback                  -- ^ Rollback in progress
