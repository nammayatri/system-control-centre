...
module NammaAP.Products.Autopilot.Sync
...
import NammaAP.Products.Autopilot.Queries.ProductService (findProductByName, getProductSyncCluster)
import NammaAP.Products.Autopilot.Queries.ReleaseTracker (insertReleaseEvent)
import NammaAP.Core.Config (Config (..))
import NammaAP.Core.Config.Runtime (isSyncClusterEnabled)
...
import NammaAP.Products.Autopilot.Types.Target.Kubernetes
  ( K8sReleaseContext(revert, syncXForwardedEmail, syncXPomeriumJwt, dockerImage, syncClusterUdf2)
...
triggerSyncIfEnabled cfg db tracker mts = do
  ...
  if null syncUrl
    then insertReleaseEvent db (releaseId tracker) "BUSINESS" "SYNC_SKIPPED" (String "No SYNC_CLUSTER_URL configured")
      ...
      insertReleaseEvent db (releaseId tracker) "BUSINESS" "SYNC_GATE_CHECK"
        (object [ "syncEnabled" .= syncEnabled
                , "udf1" .= udf1Flag
      ...
      if syncEnabled && udf1Flag && hasSyncCluster
        then do
          insertReleaseEvent db (releaseId tracker) "BUSINESS" "SYNC_TRIGGERED"
          ...
          pure ()
        else insertReleaseEvent db (releaseId tracker) "BUSINESS" "SYNC_SKIPPED"
               (object [ "syncEnabled" .= syncEnabled
                       , "udf1" .= udf1Flag
                       , "hasSyncCluster" .= hasSyncCluster
                       ])
...
createTrackerForSyncCluster cfg db tracker mts targetCluster = do
  ...
  insertReleaseEvent db (releaseId tracker) "BUSINESS" "SYNC_REQUEST"
    (object [ "url" .= url
            , "cluster" .= targetCluster
            , "auth_mode" .= authMode
  ...
  case result of
    Left e -> do
      insertReleaseEvent db (releaseId tracker) "BUSINESS" "SYNC_FAILED"
        (object [ "error" .= show e
                , "url" .= url
                , "auth_mode" .= authMode
                ])
      ...
      case retryResult of
        Right (ExitSuccess, out2, _) ->
          insertReleaseEvent db (releaseId tracker) "BUSINESS" "SYNC_RESPONSE"
            (object ["status" .= ("SUCCESS_ON_RETRY" :: Text), "body" .= out2, "url" .= url])
        Right (ExitFailure code2, _, err2) ->
          insertReleaseEvent db (releaseId tracker) "BUSINESS" "SYNC_FAILED_RETRY"
      ...
      case retryResult of
        Right (ExitSuccess, out2, _) ->
          insertReleaseEvent db (releaseId tracker) "BUSINESS" "SYNC_RESPONSE"
            (object ["status" .= ("SUCCESS_ON_RETRY" :: Text), "body" .= out2, "url" .= url])
        _ ->
          insertReleaseEvent db (releaseId tracker) "BUSINESS" "SYNC_FAILED_FINAL"
...
triggerRevertSyncIfEnabled cfg db tracker mts = do
  ...
  if not isRevert || not udf1Flag || not hasGlobalId || null syncUrl
    then pure ()
      ...
      let mSyncCluster = mProduct >>= getProductSyncCluster
          hasSyncCluster = maybe False (not . T.null) mSyncCluster
      insertReleaseEvent db (releaseId tracker) "BUSINESS" "REVERT_SYNC_GATE_CHECK"
        (object [ "syncEnabled" .= syncEnabled
                , "udf1" .= udf1Flag
      ...
      if syncEnabled && hasSyncCluster
        then do
          ...
          insertReleaseEvent db (releaseId tracker) "BUSINESS" "REVERT_SYNC_TRIGGERED"
            (String ("Triggering revert sync for global_id=" <> gid))
          _ <- forkIO $ revertTrackerSyncCluster cfg db tracker mts gid
          pure ()
        else insertReleaseEvent db (releaseId tracker) "BUSINESS" "REVERT_SYNC_SKIPPED"
...
revertTrackerSyncCluster cfg db tracker mts gid = do
  let rawUrl = syncClusterUrl cfg
      ...
      mCtx = getK8sContext mts
      (authArgs, authMode) = buildSyncAuthArgs cfg mCtx
      curlArgs = ["-s", "-X", "PUT", url
                 , "-H", "Content-Type: application/json"
                 , "-H", "Connection: close"
                 , "--max-time", "30"
                 ] <> authArgs
  insertReleaseEvent db (releaseId tracker) "BUSINESS" "REVERT_SYNC_REQUEST"
    (object ["url" .= url, "global_id" .= gid, "auth_mode" .= authMode])
  result <- try (readProcessWithExitCode "curl" curlArgs "") :: IO (Either SomeException (ExitCode, String, String))
  case result of
    Right (ExitSuccess, out, _) ->
      insertReleaseEvent db (releaseId tracker) "BUSINESS" "REVERT_SYNC_RESPONSE"
        (object ["status" .= ("SUCCESS" :: Text), "body" .= out, "url" .= url])
    Right (ExitFailure code, _, err) -> do
      insertReleaseEvent db (releaseId tracker) "BUSINESS" "REVERT_SYNC_FAILED"
      ...
      case retryResult of
        Right (ExitSuccess, out2, _) ->
          insertReleaseEvent db (releaseId tracker) "BUSINESS" "REVERT_SYNC_RESPONSE"
            (object ["status" .= ("SUCCESS_ON_RETRY" :: Text), "body" .= out2, "url" .= url])
        _ ->
          insertReleaseEvent db (releaseId tracker) "BUSINESS" "REVERT_SYNC_FAILED_FINAL"
            (object ["url" .= url, "message" .= ("Both attempts failed" :: Text)])
    Left e -> do
      insertReleaseEvent db (releaseId tracker) "BUSINESS" "REVERT_SYNC_FAILED"
    ...
    then pure ()
      ...
      if syncEnabled && hasSyncCluster
        then do
          ...
          insertReleaseEvent db (releaseId tracker) "BUSINESS" "IMMEDIATE_REVERT_SYNC_REQUEST"
            (object ["url" .= url, "global_id" .= gid, "auth_mode" .= authMode])
          _ <- forkIO $ do
            result <- try (readProcessWithExitCode "curl" curlArgs "") :: IO (Either SomeException (ExitCode, String, String))
            case result of
              Right (ExitSuccess, out, _) ->
                insertReleaseEvent db (releaseId tracker) "BUSINESS" "IMMEDIATE_REVERT_SYNC_RESPONSE"
                  (object ["status" .= ("SUCCESS" :: Text), "body" .= out, "url" .= url])
              Right (ExitFailure code, _, err) -> do
                insertReleaseEvent db (releaseId tracker) "BUSINESS" "IMMEDIATE_REVERT_SYNC_FAILED"
                  (object ["exitCode" .= code, "error" .= err, "url" .= url])
                -- Retry once
                retryResult <- try (readProcessWithExitCode "curl" curlArgs "") :: IO (Either SomeException (ExitCode, String, String))
                case retryResult of
                  Right (ExitSuccess, out2, _) ->
                    insertReleaseEvent db (releaseId tracker) "BUSINESS" "IMMEDIATE_REVERT_SYNC_RESPONSE"
                      (object ["status" .= ("SUCCESS_ON_RETRY" :: Text), "body" .= out2, "url" .= url])
                  _ ->
                    insertReleaseEvent db (releaseId tracker) "BUSINESS" "IMMEDIATE_REVERT_SYNC_FAILED_FINAL"
                      (object ["url" .= url, "message" .= ("Both attempts failed" :: Text)])
              Left e -> do
                insertReleaseEvent db (releaseId tracker) "BUSINESS" "IMMEDIATE_REVERT_SYNC_FAILED"
                  (object ["error" .= show e, "url" .= url])
