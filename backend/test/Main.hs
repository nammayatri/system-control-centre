...
testPermissions = do
    ...
    assertEqual "Manager has allPerms minus 2 edit perms"
        (length allPerms - 2) (length managerPerms)
    ...
    assertEqual "allPermissionsText count matches allPermissions"
        (length allPerms) (length allTextPerms)
    ...
    assertEqual "GRANT adds new permission"
        ["RELEASE_VIEW", "RELEASE_CREATE", "RELEASE_APPROVE"]
        combined
    assertEqual "DENY removes permission"
